...
#var seed_type = 1
@export var typePlant = 1 #Tipo de semilla que recogimos. #1 = Carrot, #2 = Onion



func _ready() -> void:
	$AnimatedSprite2D.play(Global.animation_names[typePlant])



func _on_area_2d_input_event(viewport: Node, event: InputEvent, shape_idx: int) -> void:
	if Input.is_action_just_pressed("click"): #Si se da click en el colider
		Global.plantSelected = typePlant
		selected = true
